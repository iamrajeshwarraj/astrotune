...
async function getLyricsFromLrcLib(trackName, artistName, trackDurationInSeconds) {
    // console.log(`[getLyricsFromLrcLib] Initial: trackName='${trackName}', artistName='${artistName}', duration=${trackDurationInSeconds}s`);
    try {
        let cleanedTrackName = trackName.toLowerCase().replace(/\b(ft|feat)\b\.?/g, '').replace(/\b(official|audio|video|lyrics|theme|soundtrack|music|full version|hd|4k|visualizer|radio edit|live|remix|mix|extended|cover|parody|performance|version|unplugged|reupload|lyric video|music video|audio only|instrumental)\b/gi, "").replace(/\(.*?\)|\[.*?\]|\{.*?\}/g, "").replace(/\s*[-_/|—]\s*/g, " ").replace(/[^\w\s&']/g, "").replace(/\s+/g, " ").trim();
        let cleanedArtistName = artistName.toLowerCase().replace(/\b(ft|feat)\b\.?/g, '').replace(/\b(topic|vevo|records|label|productions|entertainment|ltd|inc|band|dj|composer|performer|official)\b/gi, "").replace(/ x /gi, " & ").replace(/\(.*?\)|\[.*?\]|\{.*?\}/g, "").replace(/\s*[-_/|—]\s*/g, " ").replace(/[^\w\s&']/g, "").replace(/\s+/g, " ").trim();
        
        if (!cleanedArtistName && artistName) cleanedArtistName = artistName.trim().toLowerCase();
        if (!cleanedTrackName && trackName) cleanedTrackName = trackName.trim().toLowerCase();

        if (!cleanedTrackName || !cleanedArtistName) { return null; }

        let responseData = null;

        if (typeof trackDurationInSeconds === 'number' && !isNaN(trackDurationInSeconds) && Number.isFinite(trackDurationInSeconds) && trackDurationInSeconds > 0) {
            const paramsWithDuration = { track_name: cleanedTrackName, artist_name: cleanedArtistName, album_name: '', duration: trackDurationInSeconds };
            try {
                const res = await axios.get(`https://lrclib.net/api/get`, { params: paramsWithDuration, timeout: 7000 });
                if (res.data && (res.data.syncedLyrics || res.data.plainLyrics)) { responseData = res.data; }
            } catch (error) { /* Continue */ }
        }

        if (!responseData) {
            const paramsWithoutDuration = { track_name: cleanedTrackName, artist_name: cleanedArtistName, album_name: '' };
            try {
                const res = await axios.get(`https://lrclib.net/api/get`, { params: paramsWithoutDuration, timeout: 7000 });
                if (res.data && (res.data.syncedLyrics || res.data.plainLyrics)) { responseData = res.data; }
            } catch (error) { console.error("[getLyricsFromLrcLib] ❌ Error during request without duration:", error.message); }
        }
        return responseData ? (responseData.syncedLyrics || responseData.plainLyrics) : null;
    } catch (error) { console.error("[getLyricsFromLrcLib] ❌ Unexpected error in function:", error.message); return null; }
}

// Modified signature to accept originalAuthorId
async function showLiveLyrics(client, channel, player, config, originalAuthorId) { 
    if (!player || !player.queue.current) { const noSongEmbed = new EmbedBuilder().setColor(config.embedColor || "#FF0000").setDescription("🚫 **No song is currently playing.**"); return channel.send({ embeds: [noSongEmbed] }); }

    const track = player.queue.current;
    const trackDurationMs = track.duration; 
    let trackDurationSecForAPI = (typeof trackDurationMs === 'number' && !isNaN(trackDurationMs) && trackDurationMs > 0) ? Math.floor(trackDurationMs / 1000) : null;
    const lyricsText = await getLyricsFromLrcLib(track.title, track.author, trackDurationSecForAPI);

    if (!lyricsText) { const noLyricsEmbed = new EmbedBuilder().setColor(config.embedColor || "#FF0000").setDescription("❌ **Lyrics not found for this track!**"); return channel.send({ embeds: [noLyricsEmbed] }); }

    const lines = lyricsText.split('\n').map(line => typeof line === 'string' ? line.trim() : '').filter(Boolean); 
    const songDurationSecTotal = (typeof trackDurationMs === 'number' && !isNaN(trackDurationMs) && trackDurationMs > 0) 
    ...
    let lyricsMessage;
    try { lyricsMessage = await channel.send({ embeds: [initialEmbed], components: [row] });
    } catch (sendError) { console.error("[Lyrics Command] Failed to send initial lyrics message:", sendError); return; }

    const guildId = player.guildId || player.guild;
    if (client.guildTrackMessages && guildId) { if (!client.guildTrackMessages.has(guildId)) { client.guildTrackMessages.set(guildId, []); } client.guildTrackMessages.get(guildId).push({ messageId: lyricsMessage.id, channelId: channel.id, type: 'lyrics' }); }
    ...
    const updateLyrics = async () => {
        if (!player || !player.queue.current || player.queue.current.identifier !== track.identifier || (lyricsMessage && lyricsMessage.deleted)) {
            clearInterval(lyricsInterval);
            if (lyricsMessage && !lyricsMessage.deleted) { lyricsMessage.delete().catch(() => {}); } return;
        }
        ...
        if (lyricsMessage && !lyricsMessage.deleted) {
            ...
            catch (editError) { console.error("[UpdateLyrics] Error editing lyrics message:", editError.message, `Code: ${editError.code}`); if(editError.code === 10008 || editError.httpStatus === 404) { clearInterval(lyricsInterval); } }
        } else { clearInterval(lyricsInterval); }
    };

    const LYRICS_INTERVAL_MS = 4500; 

    if (lines.length > 0) { lyricsInterval = setInterval(updateLyrics, LYRICS_INTERVAL_MS); updateLyrics(); }
    else { initialEmbed.setDescription("Lyrics found, but format is unusual or empty."); if (lyricsMessage && !lyricsMessage.deleted) { await lyricsMessage.edit({ embeds: [initialEmbed], components: [] }); } }
    ...
    // Filter for the MessageComponentCollector
    const lyricsButtonFilter = async (interaction) => {
        if (interaction.user.id === originalAuthorId) {
            return true; // Allow interaction from the original command author
        }
        // Deny interaction for others and send an ephemeral message
        const permissionEmbed = new EmbedBuilder()
            .setColor(config.embedColor || "#FF0000") 
            .setDescription(`🚫 Only the user who started the lyrics can use these buttons.`);
        ...
        try {
            ...
            // console.error("Failed to send ephemeral permission reply for lyrics buttons:", e);
        }
        return false;
    };

    const collector = lyricsMessage.createMessageComponentCollector({ 
        filter: lyricsButtonFilter, // Apply the filter
        time: remainingDurationMs 
    });

    collector.on('collect', async i => {
        if (!player || !player.queue.current) { collector.stop("player_stopped"); return; }
        // No need to deferUpdate if the filter already replied for denied users.
        // For allowed users, deferUpdate is handled by the button actions.
        // await i.deferUpdate(); // This would be for all interactions if not filtered

        // Deferring update here for the allowed user.
        // If not deferred, and an action takes time, interaction might fail.
        try {
            await i.deferUpdate();
        } catch (deferError) {
            // console.error(`[Collector] Failed to defer update for '${i.customId}':`, deferError);
            // If defer fails, the interaction might already be too old.
            return; 
        }
    
        if (i.customId === "lyrics_stop") {
            clearInterval(lyricsInterval); collector.stop("user_stopped_lyrics");
    ...
    collector.on('end', (collected, reason) => {
        ...
        if (client.guildTrackMessages && guildId) { const messages = client.guildTrackMessages.get(guildId); if (messages) { const index = messages.findIndex(m => m.messageId === lyricsMessage.id); if (index > -1) { messages.splice(index, 1); } } }
    });
}
// --- End of functions ---
...
module.exports = {
  ...
  // Modified execute to pass message.author.id
  execute: async (client, message, args, commandName, prefix, db, config) => {
    const player = client.manager.players.get(message.guild.id);
    if (!player || !player.queue.current) { return message.reply({ embeds: [ new client.embed().setColor(config.embedColor || "#FF0000").desc("❌ **No song is currently playing!**"), ], }); }
    // Pass original message author's ID to the lyrics display function
